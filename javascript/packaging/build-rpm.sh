...
# ships CommonJS source only (no vendored node_modules/). Identical
# across every RPM family, no distro-specific flag needed.
set -euo pipefail
...
VERSION="$(../scripts/compute-version.sh rpm "${PACKAGE_JSON_VERSION:-0.1.0}")"
if [ -z "$VERSION" ]; then
...
mkdir -p "$DESTDIR"
cp -r src package.json README.md ../LICENSE "$DESTDIR/"
...
cp packaging/rpm/nodejs-multilang.spec "${RPMBUILD_ROOT}/SPECS/"

rpmbuild --define "_topdir ${RPMBUILD_ROOT}" --define "version ${VERSION}" \
    -ba "${RPMBUILD_ROOT}/SPECS/nodejs-multilang.spec"
