...
# `php83`/`php83-pdo` here) needs a real index to search, or it fails
# with "no such package" even for a package that genuinely exists in the
# repo. `apk update` fetches that index once, before handing off to
# `abuild`.
apk update
