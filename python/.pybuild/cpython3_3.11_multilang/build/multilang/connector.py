...
live in source or config files:
    ...
    MULTILANG_DB_BACKEND    sqlite | postgres | mysql
...
_SUPPORTED = ("sqlite", "postgres", "mysql")
...
def db_connector(backend=None, **credentials):
    ...
    Args:
        backend: One of "sqlite", "postgres", "mysql". Defaults to the
            MULTILANG_DB_BACKEND environment variable if not given.
        **credentials: Backend-specific connection parameters. Any key not
            ...
            For sqlite: `path`. For postgres/mysql: `host`, `port`, `user`,
            `password`, `database`.

    Returns:
        A connected Backend instance (SQLiteBackend, PostgresBackend, or
        MySQLBackend) with ensure_schema() already called, so the `strings`
        table is guaranteed to exist before the caller uses it.
