...
# so running this there is not just redundant but actively fails --
# ensurepip on openSUSE isn't usable against a Python that already has a
# distro-packaged pip.
%{suse_python} -m ensurepip --upgrade >/dev/null 2>&1
%endif
# No --break-system-packages (PEP 668): neither Leap 15's
# ensurepip-bootstrapped pip nor Tumbleweed's zypper-packaged python3-pip
# actually accepts that flag ("no such option") -- whatever build of pip
# each ships doesn't carry that patch, so it's not just unnecessary here,
# it's a hard argument-parsing error on both.
